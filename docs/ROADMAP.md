...
| 4. 랜딩 개선 + 익명 데모 | 1일 | `/pricing` + 법률 페이지 + 섹션 분할 + `/demo` (IP 쿼터) | 모든 섹션 렌더 + "Pro 시작하기" → `/signup?plan=pro` + 비로그인 `/demo`에서 URL 1건 분석 가능 |
...
- 가격 표기 "VAT 별도" 명시
- `src/app/demo/page.tsx` — 비로그인 익명 체험. URL 입력 → 결과까지 한 번. middleware 보호 라우트에서 제외.
  - 쿼터: IP당 1일 1건 (Upstash 미도입 단계라 우선 in-memory 또는 Vercel Edge Config; 남용 막는 정도면 충분)
  - 백엔드: `/api/scrape`·`/api/analyze`는 middleware로 보호되므로 `/api/demo/analyze` 별도 엔드포인트 신설(미들웨어 화이트리스트). `APIFY_TOKEN` 미설정 시 `generateDemoReviews()` 폴백을 그대로 활용 → 실 스크래핑 비용 0
  - 결과 페이지 하단에 "내 상품 추적하려면 가입" CTA → `/signup`
  - 익명 결과는 DB 영구 저장하지 않음 (또는 `user_id IS NULL` + 24h TTL cron 정리)
...
- `/share/[token]` 공유 스냅샷 (`/demo` 자체는 Phase 4로 이관)
- 카카오 알림톡 (결제 고지)
